...
multioff_dir = dataset_dir.joinpath("MultiOFF_Dataset")
multioff_feature_dir = multioff_dir.joinpath("features")
multioff_split_dir = multioff_dir.joinpath("Split Dataset")

label_remap = {
    "offensive": "offensive",
    "Non-offensiv": "harmless"
}

class MultiOffFineTuneDataset(Dataset):
    def __init__(self, split='train,valid', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        for source in self.sources:
            _data_info_dicts = []
            data_info_path = multioff_split_dir.joinpath(f'{source}_meme_dataset.csv')
            with open(data_info_path, encoding="utf-8") as f:
                for row in csv.DictReader(f):
                    row["img_id"] = row.pop("image_name")
                    row["sent"] = row.pop("sentence")
                    row["question_id"] = row["img_id"].split(".")[0]
                    row["label"] = label_remap[row["label"]]
        ...
        self.featname_to_h5 = multioff_feature_dir.joinpath(f'all_{args.feature_type}.h5')
    ...
    def __getitem__(self, idx):
        ...
        prompt = self.args.qprompt if self.args.qprompt else "multioff: {}"
...
def get_loader(args, split='train', mode='train',
    ...
    dataset = MultiOffFineTuneDataset(
        split,
        rank=gpu,
        topk=topk,
        verbose=verbose,
    ...
    _dset = MultiOffDataset(split, verbose)
    loader.evaluator = MultiOffEvaluator(_dset)
    loader.task = 'gqa'

    return loader


class MultiOffDataset:
    """
    ...
    def __init__(self, splits: str, verbose=True):
        ...
        for split in self.splits:
            _data_info_dicts = []
            data_info_path = multioff_split_dir.joinpath(f'{split}_meme_dataset.csv')
            with open(data_info_path, encoding="utf-8") as f:
                for row in csv.DictReader(f):
                    row["img_id"] = row.pop("image_name")
                    row["sent"] = row.pop("sentence")
...
class MultiOffEvaluator:
    def __init__(self, dataset: MultiOffDataset):
    ...
    def evaluate(self, quesid2ans: dict):
        ...
        labels = list(label_remap.values())
